...
// PolicyRequirement spec for fixture building. Enum-typed wherever
// the production code has a closed set (policy floor, family set,
// recovery model, provider classes) — lets fixture authors pick up
// the same symbols the runtime parser validates against and rules
// out typos ("self-service" vs "self_service") at compile time.
//
// Producer canonical CBOR bytes are generated by
// encode_policy_requirement() below; RegistryEntrySpec picks up both
// the bytes and the hash automatically when policy_requirement is
// populated. Negative tests that need to inject unknown enum values
// ("f7", "chewy_hardware_dongle") or malformed shapes use the
// MapBuilder path directly rather than going through this spec.
struct PolicyRequirementSpec {
    ...
    VMPilot::Runtime::Provider::RecoveryModel require_recovery_model{
        VMPilot::Runtime::Provider::RecoveryModel::SelfService};
    std::vector<VMPilot::Runtime::Provider::ProviderClass>
